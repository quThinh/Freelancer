...
const accountInfo = async (req, res, next) => {
  ...
  if (user_type === 'client') {
    const currentUser = await User.findOne({ email: emailLogIn, del_flag: false, type: "client" })
      ...
      .populate('category')
      .lean();
    console.log(currentUser, emailLogIn,)
...
const password = async (req, res, next) => {
  ...
  if (old_password === new_password) {
    ...
    try {
      const new_hashed_password = hashPassword(new_password,10);
...
const profile = async (req, res, next) => {
  ...
  console.log(phone, fullname, birthday, address, introduction, category, skill, social_media_contact)
